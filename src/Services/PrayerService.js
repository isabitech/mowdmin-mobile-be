...
class PrayerService {
  ...
  async createPrayer(data) {
    ...
    await this.notify(
      data.userId,
      "New Prayer Created",
      `Your prayer '${data.title}' was created.`,
      "prayer",
      { prayerId: prayer.id },
    );
    return prayer;
  ...
  async createFromRequest(requestId, adminId) {
    const PrayerRequestService = (await import("./PrayerRequestService.js"))
      .default;
  ...
  async commentPrayer(id, userId, comment) {
    ...
    if (prayer.userId && prayer.userId !== userId) {
      await this.notify(
        prayer.userId,
        "New Comment on Prayer",
        `Your prayer received a new comment.`,
        "comment",
        { prayerId: prayer.id, comment },
      );
    }
    return newComment;
  }
  async getComments(prayerId, pagination = {}) {
    const comments = await PrayerCommentRepository.findAllByPrayerId(
      prayerId,
      pagination,
    );
  ...
  async deleteComment(commentId, userId) {
    ...
    if (!comment) return { success: false, message: "Resource not found" };
  ...
  async getAll(userId, pagination = {}) {
    ...
    if (userId) {
      const likedPrayerIds =
        await PrayerLikeRepository.findLikedPrayerIdsByUserId(userId);
